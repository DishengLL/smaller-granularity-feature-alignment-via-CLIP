...
from utils.health_multimodal.image.data.io import load_image
from utils.health_multimodal.image.data.transforms import infer_resize_params
from utils.health_multimodal.image.model.model import BaseImageModel
...
class ImageInferenceEngine:
    ...
    @torch.no_grad()
    def get_projected_global_embedding(self, image) -> torch.Tensor:
        ...
        """
        projected_img_emb = None
        if isinstance(image, Path):
          input_image, _ = self.load_and_transform_input_image(image, self.transform)
